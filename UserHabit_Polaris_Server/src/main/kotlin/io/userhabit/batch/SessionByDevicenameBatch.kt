...
import io.userhabit.batch.indicators.IndicatorSessionByDevicename
import io.userhabit.polaris.service.SessionService
import reactor.core.publisher.Mono
import java.time.ZonedDateTime

/**
 * indicator_session_by_devicename
 * 세션 수 (디바이스 이름별)
 * sco
 * @author LeeJaeun
 */
object SessionByDevicenameBatch {
...
[
  ...
  {"#merge": {
    "into": "${IndicatorSessionByDevicename.COLLECTION_NAME_PT10M}",
    "on": ["st", "ai", "av", "dn"],
    "whenMatched": [
      {"#addFields" : {
        "st" : "#st",
...
[
  ...
  {"#replaceWith":{ "#mergeObjects": [ {
	...
	{"#lookup":{
    "from": "${IndicatorSessionByDevicename.COLLECTION_NAME_PT10M}",
    "let": { "stzd": "#stzd", "ai" : "#ai", "av" : "#av", "dn" : "#dn"},
    "pipeline": [
      { "#match":
        { "#expr":
          { "#and": [
  ...
  {"#merge": {
    "into": "${IndicatorSessionByDevicename.COLLECTION_NAME_PT24H}",
    "on": ["stz", "ai", "av", "dn"],
    "whenMatched": "merge",
    "whenNotMatched": "insert" }}
]			
    """
		return BatchUtil.run(object : Any(){}.javaClass, query, IndicatorSessionByDevicename.COLLECTION_NAME_PT10M, fromDt, toDt)
	}
}