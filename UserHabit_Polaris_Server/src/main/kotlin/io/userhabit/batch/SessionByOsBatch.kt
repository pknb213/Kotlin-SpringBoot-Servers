...
import io.userhabit.batch.indicators.IndicatorSessionByOs
import io.userhabit.polaris.service.SessionService
import reactor.core.publisher.Mono
import java.time.ZonedDateTime

/**
 * indicator_session_by_os 컬렉션
 * 세션 수 (os별)
 * @author LeeJaeun
 */
object SessionByOsBatch {
...
[
  ...
  {"#merge": {
    "into": "${IndicatorSessionByOs.COLLECTION_NAME_PT10M}",
    "on": ["st", "ai", "av", "dov"],
    "whenMatched": [
      {"#addFields" : {
        "st" : "#st",
...
[
  ...
  {"#replaceWith":{ "#mergeObjects": [ {
	...
	{"#lookup":{
    "from": "${IndicatorSessionByOs.COLLECTION_NAME_PT10M}",
    "let": { "stzd": "#stzd", "ai" : "#ai", "av" : "#av", "dov" : "#dov"},
    "pipeline": [
      { "#match":
        { "#expr":
          { "#and": [
  ...
  {"#merge": {
    "into": "${IndicatorSessionByOs.COLLECTION_NAME_PT24H}",
    "on": ["stz", "ai", "av", "dov"],
    "whenMatched": "merge",
    "whenNotMatched": "insert" }}
]			
    """
		return BatchUtil.run(object : Any(){}.javaClass, query, IndicatorSessionByOs.COLLECTION_NAME_PT10M, fromDt, toDt)
	}
}