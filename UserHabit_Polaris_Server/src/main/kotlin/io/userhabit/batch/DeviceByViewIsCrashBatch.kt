...
import io.userhabit.batch.indicators.IndicatorDeviceByViewIsCrash
...
/**
 * 크래시 디바이스 지표
 * 본 indicator_device_by_view_crash 컬렉션의 키 및 인덱스
 * 10분 = st, ai, av, vhi, di(10분까지만 유지)
 * 1일 = stz, ai, av, vhi
 * dco = DeviceCOunt
 * @author cjh
 */
object DeviceByViewIsCrashBatch {
...
[
  ...
  {"#merge": {
    "into": "${IndicatorDeviceByViewIsCrash.COLLECTION_NAME_PT10M}",
    "on": ["st", "ai", "av", "vhi", "di"],
    "whenMatched": [
      {"#addFields" : {
        "st" : "#st",
...
[
  ...
  {"#lookup":{
    "from": "${IndicatorDeviceByViewIsCrash.COLLECTION_NAME_PT10M}",
    "let": { "stzd": "#stzd", "ai" : "#ai", "av" : "#av", "vhi" : "#vhi", "di" : "#di"},
    "pipeline": [
      { "#match":
        { "#expr":
          { "#and": [
  ...
  {"#merge": {
    "into": "${IndicatorDeviceByViewIsCrash.COLLECTION_NAME_PT24H}",
    "on": ["stz", "ai", "av", "vhi"],
    "whenMatched": "merge",
    "whenNotMatched": "insert" }}
]
    """
    return BatchUtil.run(object : Any(){}.javaClass, query, IndicatorDeviceByViewIsCrash.COLLECTION_NAME_PT10M, fromDt, toDt)
  }
}
